...
const drawChartBook = async (subject, startIndex = 0) => {
	...
	if (cdata.error) {
		...
		cbookContainer.innerHTML = `<div class='prompt'>ツNetwork problem!</div>`
	} else {
...
const drawListBook = async () => {
	if (searchBooks.value != '') {z
		...
		if (data.error) {
			bookContainer.innerHTML = `<div class='prompt'>imit exceeded! Try after some time</div>`
